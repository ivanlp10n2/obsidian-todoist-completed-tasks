...
import { FetchTasksDomain } from 'src/constants/fetchTasks';
...
function buildRenderText(task: neededToRender, indentLevel: number = 2): string {
    ...
    const newRender: string =
        ...
        `tags: [todoist, ${task.projectName ?? 'null'}, ${task.completedAt ? 'done' : 'inprogress'}]` + `\n` +
        `${metaTag}` +
        `\n` +
        `## ${task.content}\n` +
        `### Overview\n` +
...
describe("formatTasks", () => {
	describe("prepareTasksForRendering", () => {
		it("should format tasks for rendering", () => {
			...
			console.log("actual expected:", renderedTasks)
	...
	describe("renderTasksAsText", () => {
		...
		it("should render for a single task", () => {
			const result = renderTasksAsText(singleInput, inputProjects, inputSettings)
		...
		{
			"taskId": "6960733805",
			"content": "mandar msj para ver a mariel por el diente sensible",
		...
		const singleOutput: string[] = [buildRenderText
			({
				taskId: "6960733805",
				content: "mandar msj para ver a mariel por el diente sensible",
				dueAt: "2023-06-14T13:00:00",
